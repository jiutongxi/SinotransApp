...
public class PickupActivity extends Activity {
	private ImageButton pickup_return;
	private ImageButton pickup_scan;
	private ImageButton pickup_submit;
	private ImageButton pickup_takephoto;
	private EditText pickupdonum;
...
//	private EditText pickupdescribe;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.gz_activity_pickup);
		...
		welcome=(TextView)findViewById(R.id.pickupwelcode);
		welcome.setText("欢迎"+ myApp.getUsername()+"使用手机应用系统"+version + "  " + myApp.getPosition());
		pickupdonum=(EditText) this.findViewById(R.id.pickupdonu);
		pickupdonum.setTextColor(Color.BLUE);
		orderdetail = (ListView) findViewById(R.id.pickupdodetail); 
		mImageView=(ImageView) findViewById(R.id.pickup_photo); 
//		pickupdescribe=(EditText) findViewById(R.id.pickupexcdes); 
...
//			exception_reasion = (Spinner) findViewById(R.id.pickupexception);
...
//			exceptionreasion_list= AnimationUtils.loadAnimation(this,R.anim.my_anim );	
				...
				pickupdonum.setOnKeyListener(new OnKeyListener() {  
				  ...
				  if (KeyEvent.KEYCODE_ENTER == keyCode && event.getAction() == KeyEvent.ACTION_DOWN) 
				  		{  
						  			...
						  			if(!adapter.arr.contains(pickupdonum.getText().toString()))
									{
						  				if( !"".equals(pickupdonum.getText().toString())){
							  				hs_orderCode.put(pickupdonum.getText().toString(), "");
											adapter.arr.add(pickupdonum.getText().toString());
											adapter.notifyDataSetChanged();
											pickupdonum.getText().clear();
//											隐藏软键盘
											Timer timer = new Timer();   
											        timer.schedule(new TimerTask(){   
											            ...
											            public void run() {   
											                ...
											                pickupdonum.getContext().getSystemService(Context.INPUT_METHOD_SERVICE);   
											...
											Toast.makeText(PickupActivity.this, "您输入的订单号为空",
										...
										
									{
										...
										Toast.makeText(PickupActivity.this, "您输入的订单号已录入",
													Toast.LENGTH_SHORT).show();
							  		} 
						  			pickupdonum.setFocusable(true);
						  			pickupdonum.setFocusableInTouchMode(true);
						  			pickupdonum.requestFocus();
						  			pickupdonum.requestFocusFromTouch();
		...
		pickup_scan = (ImageButton) this.findViewById(R.id.pickupscan);
		pickup_scan.setOnClickListener(new OnClickListener() {	
				@Override
				public void onClick(View v) {
					//打开扫描界面扫描条形码或二维码
					 openscanIntent = new Intent(PickupActivity.this,ScanActivity.class);
					startActivityForResult(openscanIntent, REQUEST_SCAN_CODE);
				}
			});
		pickup_return = (ImageButton) this.findViewById(R.id.pickupreturn);
		pickup_return.setOnClickListener(new OnClickListener() {	
 			@Override
 			public void onClick(View v){
 				finish();
// 		Intent openIntent = new Intent(PickupActivity.this,HomeActivity.class);
// 		startActivityForResult(openIntent, 0);
 			}
 		});
//		暂时禁用拍照功能
//		pickup_takephoto = (ImageButton) this.findViewById(R.id.pickupphoto);
//		pickup_takephoto.setOnClickListener(new OnClickListener() {	
//				@Override
//				public void onClick(View v) {
//					
//					//打开拍照界面
...
//			});
//		暂时禁用拍照功能		
		pickup_submit = (ImageButton) this.findViewById(R.id.pickupsave);
		pickup_submit.setOnClickListener(new OnClickListener() {	
 			@Override
 			public void onClick(View v) {
 				//打开扫描界面扫描条形码或二维码
 				String s="";
 					...
 					{
 						Toast.makeText(PickupActivity.this, "数据保存中......",
									Toast.LENGTH_SHORT).show();
 						...
 						for(int i=0;i<orderqty;i++)
							{
 							...
 							if("success".equals(s))
 			 				{
 			 					Toast.makeText(PickupActivity.this, "数据保存成功",
 										Toast.LENGTH_SHORT).show();
// 			 					1.8秒后跳出下个Activity
 			 					Timer timer = new Timer();   
						        timer.schedule(new TimerTask(){   
 			 				...
 			 				{
 			 					Toast.makeText(PickupActivity.this, "信息保存失败"+s,
 										Toast.LENGTH_SHORT).show();
 			 					Log.i("M",s);
 			 				}	
 					}
 					else{
 						Toast.makeText(PickupActivity.this, "订单信息为空,请录入相关订单信息",
									Toast.LENGTH_SHORT).show();
 					}
			...
			String scanResult="";
				...
				scanResult = bundle.getString("result");	
					if(!adapter.arr.contains(scanResult))
					{
							...
							Toast.makeText(PickupActivity.this, "订单扫描成功，订单号为:"+scanResult,
									Toast.LENGTH_SHORT).show();

					}
					else 
					{
						Toast.makeText(PickupActivity.this, "您输入的订单号已经扫描",
										Toast.LENGTH_SHORT).show();
					}
